  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="order-dialog supplier-confirm-dialog">
        ...
        <div className="incoming-dialog-facts"><div><span>EXPECTED DELIVERY</span><strong>{order.delivery}</strong></div><div><span>DELIVERY LOCATION</span><strong>{order.location}</strong></div><div><span>ORDER VALUE</span><strong>{money(order.value)} SUI</strong></div></div>
        <div className="incoming-items-shell"><div><span>ORDER CONTENTS</span><strong>{order.items.length} LINE ITEMS</strong></div><Table className="incoming-items-table"><TableHeader><TableRow><TableHead>Product</TableHead><TableHead>Qty</TableHead><TableHead className="table-amount">Total</TableHead></TableRow></TableHeader><TableBody>{order.items.map((item) => <TableRow key={item.id}><TableCell><strong>{item.description || "Product pending"}</strong><small>{item.sku || "No SKU"} · {money(item.unitPrice)} SUI / {item.unit}</small></TableCell><TableCell>{money(item.quantity)} {item.unit}</TableCell><TableCell className="table-amount">{money(item.quantity * item.unitPrice)}</TableCell></TableRow>)}</TableBody></Table></div>
...
export default function SupplierWorkspace() {
  ...
  const steps = [
    ...
    { icon: LockKeyhole, label: "Funded", note: "30,000 SUI" },
    ...
    <div className="buyer-shell supplier-shell">
      ...
      <main className="buyer-main">
        ...
        <section className={`supplier-incoming-request request-${orderResponse}`}>
          ...
          <div className="incoming-request-value"><small>ORDER VALUE</small><strong>{money(pendingOrder.value)} <span>SUI</span></strong></div>
          <ConfirmIncomingOrder order={pendingOrder} response={orderResponse} onConfirm={confirmOrder} onRequestChanges={requestChanges} />
        </section>

        <section className="summary-rule supplier-summary"><article><span>SECURED ORDER VALUE</span><strong>70,400 <small>SUI</small></strong><p>Funds visible before dispatch</p></article><article><span>READY TO SHIP</span><strong>2</strong><p>Both orders fully funded</p></article><article><span>RELEASE READY</span><strong>26,100 <small>SUI</small></strong><p>Accepted by GreenBite</p></article><article><span>HELD IN EXCEPTION</span><strong>3,900 <small>SUI</small></strong><p>Only disputed value remains</p></article></section>

        <section id="shipments" className="supplier-focus-grid">
          <article className="work-panel focus-order supplier-focus"><div className="focus-head"><div><span className="card-label">PAYMENT SECURED · YOUR ACTION</span><h2>Prepare the funded shipment</h2></div><span className="secured-chip"><i />30,000 SUI locked</span></div><div className="order-line"><span className="supplier-avatar">GB</span><div><strong>GreenBite Foods</strong><small>PO-2471 · Cooking oil · 100 cartons</small></div><div className="order-money"><small>PAYOUT ADDRESS</small><strong>0x71F…9A2</strong></div></div><div className="mini-flow">{steps.map((step,index) => { const StepIcon = step.icon; return <div key={step.label} className={index < 2 ? "done" : index === 2 ? "current" : ""}><span><StepIcon size={14} /></span><strong>{step.label}</strong><small>{step.note}</small></div>; })}</div><div className="focus-actions supplier-actions"><p><ShieldCheck size={15} />Payout address was snapshotted when this PO was funded.</p><div><button className="evidence-button" type="button" onClick={() => setNotice("evidence")}><FileCheck2 size={15} />Attach evidence</button><DeliveryQR onGenerated={() => setNotice("qr")} /></div></div></article>

          <article id="payouts" className="work-panel supplier-payout"><span className="card-label">PARTIAL SETTLEMENT</span><h2>Accepted value moves first.</h2><div className="payout-total"><span>RELEASE READY</span><strong>26,100 <small>SUI</small></strong></div><div className="payout-split"><div><i /><span>87 accepted cartons</span><strong>26,100</strong></div><div><i /><span>13 in exception</span><strong>3,900</strong></div></div><p><Clock3 size={14} />The accepted portion can settle without waiting for the remaining dispute.</p></article>
        ...
        <section id="orders" className="second-grid supplier-second-grid"><article className="work-panel orders-panel"><div className="panel-head"><div><span className="card-label">INCOMING TRADE PIPELINE</span><h2>Incoming purchase orders</h2></div><button>View all <ArrowRight size={14} /></button></div><Table className="order-table"><TableHeader><TableRow><TableHead>Order</TableHead><TableHead>Buyer</TableHead><TableHead>Status</TableHead><TableHead className="table-amount">Value</TableHead></TableRow></TableHeader><TableBody>{displayedOrders.map((order) => <TableRow key={order[0]}><TableCell><strong>{order[0]}</strong><small>Verified counterparty</small></TableCell><TableCell>{order[1]}</TableCell><TableCell>{order[2]}</TableCell><TableCell className="table-amount">{order[3]} SUI</TableCell></TableRow>)}</TableBody></Table></article><article id="evidence" className="work-panel activity-panel"><div className="panel-head"><div><span className="card-label">SHARED RECORD</span><h2>Supplier activity</h2></div></div><div className="activity-list"><article><span><BadgeCheck size={15} /></span><div><strong>Buyer accepted 87 cartons</strong><p>26,100 SUI is ready to release for PO-2463.</p><small>12 minutes ago</small></div></article><article><span><Camera size={15} /></span><div><strong>Evidence hash anchored</strong><p>Six dispatch photos linked to PO-2454.</p><small>Yesterday, 5:18 PM</small></div></article><article><span><WalletCards size={15} /></span><div><strong>Payment secured</strong><p>Kita Grocer funded PO-2470 in full.</p><small>Yesterday, 2:42 PM</small></div></article></div></article></section>

        <section className="supplier-record-strip"><span><PackageCheck size={17} />SETTLEMENT RECEIPT</span><p>PO-2454 · 12,400 SUI distributed through one programmable transaction.</p><a href="#">View on Sui Explorer <ExternalLink size={14} /></a></section>
