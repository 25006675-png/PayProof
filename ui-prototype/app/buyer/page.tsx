        ...
        {step === 1 ? (
          ...
          <>
            ...
            <div className="line-items-shell">
              ...
              <div className="order-total"><span><small>Draft total</small><strong>Calculated from {items.length} line {items.length === 1 ? "item" : "items"}</strong></span><b>{money(total)} <small>SUI</small></b></div>
            </div>
      ...
      <SheetContent className="review-sheet order-details-sheet">
        ...
        <div className="sheet-body order-details-body">
          <section className="detail-hero">
            <div><span>PROTECTED ORDER VALUE</span><strong>{money(order.value)} <small>SUI</small></strong></div>
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent className="review-sheet">
        ...
        <div className="sheet-body">
          <section className="sheet-card"><span>QUANTITY CHECK</span><h3>100 cartons delivered</h3><div className="quantity-split"><div><span>ACCEPTED</span><strong>87</strong><small>26,100 SUI</small></div><div><span>DAMAGED / MISSING</span><strong>13</strong><small>3,900 SUI held</small></div></div></section>
          ...
          <div className="release-box"><small>RELEASE AFTER CONFIRMATION</small><strong>26,100 SUI</strong><p>Only the disputed 3,900 SUI remains protected in escrow.</p></div>
  ...
  return (
    <div className="buyer-shell">
      ...
      <main className="buyer-main">
        {notice && <div className="app-alert"><CheckCircle2 size={17} /><span>{notice === "created" ? `${draftOrder?.id || "Order"} sent to ${draftOrder?.supplier || "the supplier"}. Funding stays disabled until they confirm.` : notice === "supplierConfirmed" ? `${draftOrder?.supplier || "The supplier"} confirmed ${draftOrder?.id || "the order"}. Your next step is to fund escrow.` : notice === "changesRequested" ? `${draftOrder?.supplier || "The supplier"} requested changes to ${draftOrder?.id || "the order"}. Review the shared terms before resending.` : "Inspection confirmed. 26,100 SUI is ready for settlement."}</span><button onClick={() => setNotice(null)}>Dismiss</button></div>}
        ...
        <section className="summary-rule"><article><span>SECURED IN ESCROW</span><strong>78,400 <small>SUI</small></strong><p>Across three funded orders</p></article><article><span>READY TO RELEASE</span><strong>26,100 <small>SUI</small></strong><p>Awaiting inspection decision</p></article><article><span>OPEN ORDERS</span><strong>{draftOrder ? 5 : 4}</strong><p>Two active suppliers</p></article><article><span>ACTION NEEDED</span><strong>2</strong><p>One inspection · one response</p></article></section>
        <section className="work-grid">
          <article className="work-panel focus-order"><div className="focus-head"><div><span className="card-label">NEEDS YOUR ATTENTION</span><h2>Delivery ready for inspection</h2></div><span className="status-chip">43h 12m left</span></div><div className="order-line"><span className="supplier-avatar">FS</span><div><strong>FreshSource Foods</strong><small>PO-2471 · Cooking oil · 100 cartons</small></div><div className="order-money"><small>ESCROWED VALUE</small><strong>30,000 SUI</strong></div></div><div className="mini-flow">{steps.map((step, index) => { const StepIcon = step.icon; return <div key={step.label} className={index === 3 ? "current" : "done"}><span><StepIcon size={14} /></span><strong>{step.label}</strong><small>{step.note}</small></div>; })}</div><div className="focus-actions"><p><ShieldCheck size={15} />Funds remain locked until your signed decision.</p><ReviewDelivery onConfirmed={() => setNotice("confirmed")} /></div></article>
